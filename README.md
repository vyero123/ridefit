...
The **violet** note is what the microphone is hearing right now, for as long as
it is sounding. It is drawn **in the target's own column, at its own pitch**,
so the vertical gap between the two is the feedback: below the target means you
are under it, above means over it, level means right. When you are on the right
note the violet draws as a *ring* around the target rather than a notehead on
top of it, so the green "correct" colour reads at full strength and neither
mark hides the other.

A pitch more than four ledger lines outside the staff is clamped to that limit
and marked with an arrow, rather than drawing twenty ledger lines and wrecking
the layout — at that distance the useful information is just "far above" or
"far below", and the note name is in the readout anyway.
...
js/
  ...
  notation/
    glyphs.js                  hand-built accidental and notehead geometry
    clefs.js                   GENERATED: Bravura clef outlines (SIL OFL)
    staff.js                   SVG staff renderer
tools/
  extract-clefs.py             regenerates clefs.js from Bravura
tests/
...
**The notation is hand-rolled, not VexFlow.** Accidentals and noteheads in
`glyphs.js` are built from explicit control points in units of one staff space,
with the origin at the pitch the glyph refers to. Scaling is a multiply, and
any part of a glyph can be animated independently — which is the reason for not
pulling in a notation library.

**The two clefs are the exception.** They were hand-drawn too, as a single
constant-width stroke through hand-placed control points, and they were wrong —
legible, but visibly off to anyone who reads music. They are now the G and F
clef outlines from **Bravura**, the SMuFL reference music font by Steinberg,
used under the **SIL Open Font License 1.1**. The full licence is in `OFL.txt`;
attribution is in `js/notation/clefs.js`, which is generated by
`tools/extract-clefs.py` and can be regenerated from the font at any time.

Nothing is shipped as a font or under the Bravura name: these are two glyph
outlines embedded as SVG path data, with the copyright notice and licence
travelling with them as the OFL requires. No font file is loaded at runtime —
two path strings are about 3 kB against ~100 kB and a webfont round trip.

The placement is correct by construction rather than by eye. SMuFL defines a
clef's origin as sitting **on the line the clef names**, so translating the
path to the G line puts the spiral's centre on the G line, and translating the
F clef to the F line makes its two dots straddle that line, exactly. The
browser suite measures both.
...
- While a note is still ringing after you have played it correctly, the overlay
  follows the target to the next column, so it briefly shows the note you are
  still holding against the note coming next. That is accurate, but it can read
  as "you are under the target" for a moment.

---

## Third-party material

The G and F clef outlines in `js/notation/clefs.js` are derived from
**Bravura**, copyright © Steinberg Media Technologies GmbH, licensed under the
**SIL Open Font License, Version 1.1**. The full licence text is in `OFL.txt`.
Bravura is the reference font for SMuFL, the Standard Music Font Layout.
<https://github.com/steinbergmedia/bravura>

Everything else in this repository is original.
