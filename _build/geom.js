...
var VVY = (function () {
  ...
  function personSvg(h, cx, Y, fill, cls) {
    var o = [];
    o.push('<g class="' + (cls || 'vvy-person') + '" fill="' + (fill || '#1f2933') + '">');
  ...
  function dogSvg(h, cx, Y, fill) {
    ...
    o.push('<rect x="' + X(-12) + '" y="' + YY(12) + '" width="' + r1(3 * u) + '" height="' + r1(12 * u) + '"/>');
    o.push('<rect x="' + X(-6) + '" y="' + YY(12) + '" width="' + r1(3 * u) + '" height="' + r1(12 * u) + '"/>');
    o.push('<rect x="' + X(1) + '" y="' + YY(12) + '" width="' + r1(3 * u) + '" height="' + r1(12 * u) + '"/>');
  ...
  function catSprite(fill) {
    ...
    return '<g class="vvy-catbody" fill="' + c + '">' +
      ...
      '<path d="M-6 -4 Q-11 -3 -10.5 -9.5" fill="none" stroke="' + c + '" stroke-width="1.5" stroke-linecap="round"/>' +
      '<rect x="-4.6" y="-1.6" width="1.6" height="1.8"/><rect x="2.4" y="-1.6" width="1.6" height="1.8"/>' +
  ...
  /* e = effective cfg; vx = x of front bumper (inches); Y = y mapper */
  function vehicleSvg(e, vx, Y, role, animate, solid, spots) {
    ...
    d.push('Z');
    o.push('<path d="' + d.join(' ') + '" fill="' + pal.body + '" stroke="' + pal.stroke + '" stroke-width="0.45" stroke-linejoin="round"' +
      (role === 'B' ? ' stroke-dasharray="2.2 1.4"' : '') + '/>');
    if (body.glass) {
      var gp = [];
      for (i = 0; i < body.glass.length; i++) { gp.push(X(body.glass[i][0]) + ',' + YY(body.glass[i][1])); }
      o.push('<polygon points="' + gp.join(' ') + '" fill="' + pal.glass + '" opacity="0.8"/>');
    }
    ...
    var wy = Y(wr);
    o.push(wheelSvg(fx, wy, wr, pal, animate));
    o.push(wheelSvg(rx, wy, wr, pal, animate));
    o.push('</g>');
    return o.join('');
  }
  ...
  function renderScene(vehicles, personIn, metric, opts) {
    ...
    o.push(vehicleSvg(A, vxA, Y, 'A', !!opts.animate, false, spots));
    if (inter) { o.push('</g>'); }
    if (inter && !B) { o.push(qmarkSvg(vxA + LA + gap, 0, A.height, Y)); }
    if (B) { o.push(vehicleSvg(B, vxB, Y, 'B', !!opts.animate, side)); }
    ...
    for (i = 0; i < fam.length; i++) {
      fc = fx0 + fam[i].w / 2;
      if (fam[i].kind === 'dog') { o.push('<g pointer-events="none">' + dogSvg(fam[i].h, fc, Y, '#6b4f3a') + '</g>'); }
      else {
        if (inter) { o.push(hitOpen('crew', 'Edit this rider', fc - Math.max(fam[i].w / 2, 9) - 3, Y(fam[i].h) - 4, Math.max(fam[i].w, 18) + 6, fam[i].h + 6)); }
        o.push(personSvg(fam[i].h, fc, Y, fam[i].kind === 'kid' ? '#5f7d95' : '#3d4f60', 'vvy-fam vvy-' + fam[i].kind));
        if (inter) { o.push('</g>'); }
      }
      fx0 += fam[i].w + 4;
    }
    ...
    if (nC > 0) {
      ...
      o.push('<g class="vvy-cats" pointer-events="none" data-spots="[' + spotJson.join(',') + ']">');
      for (i = 0; i < nC; i++) {
        var cx0 = r1(mL + pw + 2 + i * 13), cy0 = Y(0);
        o.push('<g class="vvy-cat" transform="translate(' + cx0 + ' ' + cy0 + ')" data-i="' + i + '">' + catSprite(i % 3 === 0 ? '#4a4a52' : (i % 3 === 1 ? '#8a6d4b' : '#b8b2a7')) +
          meowSvg() + '</g>');
      }
      o.push('</g>');
    }
